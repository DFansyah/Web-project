...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id']) && $_POST['id'] != '') {
        ...
        $pdo->prepare($sql)->execute([$name, $price, $desc, $img_path, $_POST['id']]);
    } else {
        $sql = "INSERT INTO products (name, price, description, image) VALUES (?, ?, ?, ?)";
        $pdo->prepare($sql)->execute([$name, $price, $desc, $img_path]);
    }
    header("Location: admin.php"); exit;
}
...
<body class="admin-body">
    ...
    <header class="top-header">
        <div class="brand-logo">ADMIN<b>RESTO</b></div>
        
        <nav class="desktop-nav">
            <a href="index.php" target="_blank">Lihat Menu</a>
            <a href="logout.php">Keluar</a>
        </nav>
        
        <a href="logout.php" class="mobile-auth-btn btn-m-out" onclick="return confirm('Logout Admin?');">
    ...
    <main class="main-content">
        ...
        <div class="admin-form-card">
            <h3><?php echo $edit ? 'Edit Menu' : 'Tambah Menu Baru'; ?></h3>
            <form method="POST" enctype="multipart/form-data" style="margin-top:15px;">
                ...
                <button type="submit" class="btn-primary"><?php echo $edit?'Simpan Perubahan':'Upload Menu'; ?></button>
                <?php if($edit): ?> <a href="admin.php" style="display:block; text-align:center; margin-top:10px; color:#777;">Batal</a> <?php endif; ?>
            </form>
        </div>

        <h3 style="margin-bottom:15px;">Daftar Menu Aktif</h3>
        <div class="product-grid">
            ...
            <div class="product-card">
                ...
                <div class="admin-actions" style="padding:10px;">
                    ...
                    <a href="admin.php?del=<?php echo $p['id']; ?>" class="btn-action btn-delete" onclick="return confirm('Hapus menu ini?');">Hapus</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
